...
def main():
    ...
    # create_plots: Ob die Plots erstellt werden sollen (Default: True)
    # save_plots: Ob die Plots gespeichert werden sollen (Default: False)
    # forces_to_plot: Ausgewählte Kräfte (Default: alle [Fy, Fx, Fz, Mz])
    ...
    # Beispielhaft: Ordnerpfad zu den .lvm-Dateien (anpassen!)
    ...
    if usefilteredDict == True:
        current_dict = filtered_data_dict
        optional_suffix = optional_suffix + "_filtered_" + str(SVGwindowlength) + "_" + str(SVGpolyorder)
    if usefilteredDict == False:
        current_dict = sorted_data_dict
        optional_suffix = optional_suffix + "_raw"
    ...
    # Extrahiere die Kräfte je Griff aus der GUI-Auswahl
    ...
    # Falls Plots erstellt werden sollen, die Funktion zum Plotten aufrufen
    if create_plots:
        if split_fmz_var == False:
